...
class TurtlebotPlanner(Node):
    def __init__(self):
        ...
        self.rate = self.create_rate(10)  # Creatin a Rate object with a frequency of 10 Hz
        self.velocities = []  # Initializing an empty list to store velocities
        self.index = 0  # Initializing an index to keep track of the current velocity
...
def main(args=None):
    ...
    # Generating map
    map = np.zeros((200, 600, 3))
    red = np.array([255, 0, 0])
    yellow = np.array([255, 255, 0])
    black = np.array([0, 0, 0])
    clearance = 14  #int(input("Enter the clearance value:"))
    robot_radius = 15
    RPM1 = 25   #int(input("Enter the RPM1 value:"))
    RPM2 = 100  #int(input("Enter the RPM2 value:"))

    # Generating boundary clearance
    cv2.rectangle(map, pt1=(000, 000), pt2=(600, 200), color=(255, 255, 0), thickness=-1)
    cv2.rectangle(map, pt1=(000 + (clearance + robot_radius), 000 + (clearance + robot_radius)), pt2=((600 - (clearance + robot_radius)), (200 - (clearance + robot_radius))), color=(0, 0, 0), thickness=-1)

    # Generating rectangles clearance
    cv2.rectangle(map, pt1=(150 - (clearance + robot_radius), 200), pt2=(175 + (clearance + robot_radius), 100 - (clearance + robot_radius)), color=(255, 255, 0), thickness=-1)
    cv2.rectangle(map, pt1=(250 - (clearance + robot_radius), 100 + (clearance + robot_radius)), pt2=((275 + (clearance + robot_radius)), 0), color=(255, 255, 0), thickness=-1)

    # Generating rectangles obstacle
    cv2.rectangle(map, pt1=(150, 200), pt2=(175, 100), color=(255, 0, 0), thickness=-1)
    cv2.rectangle(map, pt1=(250, 100), pt2=(275, 0), color=(255, 0, 0), thickness=-1)

    # Generating circle clearance
    center_circle = (420, 120)   
    radius_circle = 60 + (clearance + robot_radius)
    ...
    while True:
        start_x = 76 #int(input("\nEnter your start x-coordinate: "))
        start_y = 100 #int(input("Enter your start y-coordinate: "))
        ...
        if ((clearance + robot_radius) <= start_x <= (map.shape[1]- (clearance + robot_radius))) and ((clearance + robot_radius) <= start_y <= (map.shape[0]-(clearance + robot_radius))):
            ...
            print("Your starts are not within boundary or on clearances. Please re-enter the start coordinates.")
            continue
    ...
    while True:
        goal_x = 540 #int(input("\nEnter your goal x-coordinate: "))
        goal_y = 100 #int(input("Enter your goal y-coordinate: "))
    ...
    max_linear_velocity = 0.05952  # Example value, adjust as needed
    max_angular_velocity = 0.38  # Example value, adjust as needed
    ...
    def calculate_velocities(path):
        ...
        for i in range(len(path) - 1):
            ...
            # Printing the calculated velocities
